...
function calculate(inp) {
  ...
  const rN=inp.insulThick*ins.rPerInch, sp=457/v.ksFactor;
  const cols=Math.max(1,Math.floor((inp.panelW-150)/sp));
  const rows=Math.max(1,Math.floor((inp.panelH-150)/sp));
  ...
  const grid=[];
  for(let r=0;r<rows;r++) for(let c=0;c<cols;c++) grid.push({x:150+c*sp,y:150+r*sp});
  return {
...
function KpiCard({label, value, unit, sub, accent, warn, icon}) {
  ...
  return (
    <div style={{
      ...
      boxShadow: accent||warn ? `0 0 20px ${glowCol}40, inset 0 1px 0 ${col}20` : "none",
    }}>
      <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:col,opacity:0.8}}/>
      ...
      <div style={{fontSize:9,color:T.ca200,textTransform:"uppercase",letterSpacing:1.5,marginBottom:8,fontWeight:500}}>{label}</div>
      <div style={{fontSize:30,fontWeight:700,color:col,fontFamily:"'DM Mono',monospace",lineHeight:1}}>
        <AnimNum value={typeof value==="string"?parseFloat(value)||value:value} suffix={unit||""} decimals={typeof value==="number"&&value%1!==0?1:0}/>
      </div>
      {sub && <div style={{fontSize:10,color:T.ca200,marginTop:6}}>{sub}</div>}
    </div>
  );
}
...
// ─── TABS ─────────────────────────────────────────────────────────────────────
const TAB_LIST = [
  {id:"sizing",     icon:"📐", label:"Sizing"},
  {id:"heatmap",    icon:"🌡",  label:"Heat Map"},
  {id:"energy",     icon:"💰", label:"Energy Savings"},
...
function TabBar({active, onChange}) {
  return (
    <div style={{display:"flex",gap:4,padding:"10px 20px",background:T.ca600,borderBottom:`1px solid ${T.ca400}`}}>
      {TAB_LIST.map(t=>{
        ...
        return (
          <button key={t.id} onClick={()=>onChange(t.id)} style={{
            ...
            color:isActive?T.cu300:T.ca200,
            fontWeight:isActive?700:400, fontSize:12, fontFamily:"'DM Sans',sans-serif",
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:T.ca800,fontFamily:"'DM Sans','Segoe UI',sans-serif",color:"#EDE8E3",fontSize:13}}>
      <style>{`
      ...
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            ...
            <div style={{fontSize:8,color:T.ca200,letterSpacing:3.5,marginTop:2,fontWeight:500}}>ALTA PERFORMANCE</div>
          </div>
        </div>
        <div style={{width:1,height:32,background:T.ca400,margin:"0 4px"}}/>
        <div style={{fontSize:11,color:T.ca200,letterSpacing:1.5,textTransform:"uppercase",fontWeight:500}}>Designer · Sizing + ROI Suite</div>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:10,alignItems:"center"}}>
          <div style={{
            ...
            <div style={{width:6,height:6,borderRadius:"50%",background:R.ok?T.green:T.red,boxShadow:`0 0 6px ${R.ok?T.green:T.red}`}}/>
            <span style={{color:R.ok?T.green:T.red}}>{R.ok?"ASHRAE Compliant":"Non-Compliant"}</span>
          </div>
          <div style={{fontSize:9,color:T.cu600,background:T.ca700,padding:"4px 10px",borderRadius:20,border:`1px solid ${T.ca400}`}}>
            V7 · ICC-ES AC422
          </div>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"284px 1fr",minHeight:"calc(100vh - 70px)"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",background:T.ca800}}>
          ...
          <div style={{padding:"8px 20px",background:T.ca700,borderTop:`1px solid ${T.ca400}`,fontSize:10,color:T.ca300,textAlign:"center"}}>
            ConnexFrame Designer V7 · Preliminary estimates only. Final design requires certified analysis per ACI 318 and ICC-ES AC422 by a licensed engineer of record.
          </div>
        </div>
      </div>
    </div>
